(() => {
  ...
  async function loadData() {
    ...
    state.emigration = data.emigration || [];
    state.locations = [...data.locations].sort((a, b) => a.order - b.order);
  }

  function renderEmigration() {
    const box = $("#emigrationList");
    if (!box) return;
    if (!state.emigration.length) {
      $("#emigrationBlock").hidden = true;
      return;
    }
    box.innerHTML = state.emigration
      .map(
        (e) => `
      <article class="emigration-item">
        <p class="emigration-period">${e.period}</p>
        <h4>${e.place}</h4>
        <p>${e.note}</p>
      </article>`
      )
  ...
  function goTo(index, opts = {}) {
    ...
    const zoom = loc.lat > 60.5 || loc.id === 10 || loc.id === 6 ? 8 : 13;
